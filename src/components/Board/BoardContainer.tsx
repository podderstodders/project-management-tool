...
export const CurrentBoardContainer: React.FC<boardContainerProps> = ({ sidebarCloser}) => {
  ...
  const notificationMessageHandler = (child: ReactNode) => {
    if(child !== undefined){
  ...
  useEffect( () => {
    if(tester69.active) { 
      setTimeout( () => {
        setTester69({active: false, message: undefined})
      }, 3000)
    }
  }, [tester69.active])
  ...
  if(!currentBoard) {
    ...
    return (
      <section className="app-main">
          ...
          <div className="list-container">
            {
              currentBoard.lists.map( (list, index) => (
                <div className="list-item notemptylist" key={index}>
                  <div className="list-item--header">
                    ...
                    { listMenuToggle.active 
                      ...
                      && 
                      <ListActionMenu boardName={currentBoard.boardName} list={list} listIndex={listMenuToggle.index} addCardToggleHandler={() => newCardToggleHandler(index)} parentCloseHandler={() => setListMenuToggle({active: false, listName: '', index: -1})} setWatching={listWatchingHandler} copyList={copyListHandler} setNotification={notificationMessageHandler}/> 
                    }
                  </div>
        ...
        <NotificationMessage isActive={tester69.active} message={tester69.message} closeHandler={() => setTester69({active: false, message: ''})}/>
      </section>
    )
